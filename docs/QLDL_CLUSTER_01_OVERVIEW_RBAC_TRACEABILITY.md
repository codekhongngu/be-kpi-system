...
- Kỳ báo cáo (period snapshot): cấu hình khi **giao báo cáo / tổng hợp** (không còn danh mục kỳ riêng)
...
| **System Admin** | Toàn bộ API quản trị + giám sát: `/users*`, `/role-groups*`, `/permissions`, `/orgs*`, `/monitoring*`, `/query*`, `/notifications/logs`, đọc toàn bộ dữ liệu nghiệp vụ theo policy | Global |
| **Data Manager** | `/field-categories*`, `/forms*`, `/assignments*`, `/summaries*`, `/monitoring*`, `/query*`, `/analytics*`, `/notifications`, `/me`, `/auth/*` | Theo scope (mặc định: toàn xã); có thể hạn chế org-tree nếu cấu hình |
...
| Mã | Tên | API chính | DB chính |
| --- | --- | --- | --- |
| FM-01 | Thiết kế biểu mẫu | `GET/POST/PATCH /field-categories`, `GET/POST/PATCH/DELETE /forms`, `POST /forms/{id}/activate|deactivate`, `POST /forms/{id}/copy`, `POST /forms/{id}/template` | `field_categories`, `forms` (`field_category_id` FK), `audit_logs` (+ object storage cho file) |
...
| Mã | Tên | API chính | DB chính |
| --- | --- | --- | --- |
| ASGN-01 | Giao báo cáo | `GET/POST /assignments`, `POST /assignments/{id}/cancel`, `POST /assignments/next-period` | `form_assignments` (period snapshot), `forms`, `organizations`, `notifications`, `audit_logs` |
...
| QRY-01 | Tra cứu | `GET /query/reports`, `GET /query/reports/{submissionId}`, `GET /query/reports/{submissionId}/export` (xem mục “Query / Tra cứu” trong **Cụm 2**) | reads: `report_submissions`, `form_assignments`, joins `forms/organizations` (period snapshot) (+ `audit_logs` khi export) |
