...
const PERMISSIONS = [
  ...
  ['field-categories.manage', 'Quản lý danh mục lĩnh vực', 'CRUD danh mục lĩnh vực biểu mẫu', 'qldl'],
...
export class DefaultRbac1863000001001 implements MigrationInterface {
  name = 'DefaultRbac1863000001001';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const roles = [
      ['SUPER_ADMIN', 'Siêu quản trị', 'Toàn quyền cấu hình & vận hành', true],
    ...
    const rolePermissions: Record<string, string[]> = {
      ...
      SYSTEM_ADMIN: [
        ...
        'forms.manage', 'field-categories.manage', 'assignments.manage', 'submissions.manage',
      ...
      DATA_MANAGER: [
        'forms.manage', 'field-categories.manage', 'assignments.manage', 'campaigns.manage', 'campaigns.dispatch',
      ...
      DATA_ENTRY: ['submissions.manage', 'notifications.read', 'reports.read', 'reports.export'],
      APPROVER: ['approvals.manage', 'approvals.department.manage', 'approvals.district.manage', 'notifications.read', 'reports.read', 'reports.export'],
    };
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DELETE FROM "role_permissions" WHERE "role_id" IN (SELECT "id" FROM "roles" WHERE "code" IN ('SUPER_ADMIN','SYSTEM_ADMIN','DATA_MANAGER','DATA_ENTRY','APPROVER'))`);
    await queryRunner.query(`DELETE FROM "roles" WHERE "code" IN ('SUPER_ADMIN','SYSTEM_ADMIN','DATA_MANAGER','DATA_ENTRY','APPROVER')`);
    await queryRunner.query(`DELETE FROM "permissions" WHERE "code" IN (${PERMISSIONS.map((p) => `'${p[0]}'`).join(',')})`);
  }
}
