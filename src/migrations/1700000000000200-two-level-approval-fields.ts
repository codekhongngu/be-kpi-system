...
export class AddTwoLevelApprovalFields1700000000000200 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Drop indexes first
    await queryRunner.dropIndex('report_submissions', 'district_approved_at');
    await queryRunner.dropIndex('report_submissions', 'district_approved_by');
    await queryRunner.dropIndex('report_submissions', 'department_approved_at');
    await queryRunner.dropIndex('report_submissions', 'department_approved_by');
    
    // Drop approval history indexes
    await queryRunner.dropIndex('approval_history', 'user_id');
    await queryRunner.dropIndex('approval_history', 'approval_level');
    await queryRunner.dropIndex('approval_history', 'submission_id');
