...
@Injectable()
export class SummaryAnalyticsQueryService {
  constructor(
  ...
  async listReports(user: User, query: QueryReportsDto) {
    ...
    if (query.deadlineTo) {
      countQb.andWhere('a.deadlineTo <= :dt', { dt: query.deadlineTo.slice(0, 10) });
    }
    if (query.q?.trim()) {
      const q = `%${query.q.trim().toLowerCase()}%`;
      countQb.andWhere(
    ...
    const items = rows.map((r) => ({
      ...
      period: { type: r.periodType, code: r.periodCode, name: r.periodName },
      status: r.status,
  ...
  async reportDetail(submissionId: string, user: User) {
    ...
    if (!s) throw new NotFoundException('Khong tim thay ban nop');
    ...
    if (user.orgId && a.orgId !== user.orgId) {
      throw new NotFoundException('Khong tim thay ban nop');
    }
    return {
      submissionId: s.id,
      assignmentId: a.id,
